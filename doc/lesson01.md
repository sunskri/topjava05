...
- Хранение в HashMap - одна из наших реализаций (такжен будет jdbc, jpa и data-jpa). Т.е. с ней лучше работать через интерфейс и сервлет не должен знать о деталях реализации (Map, DB или что-то еще), хотя, пока нет Spring, можно его там инстанциировать (слои приложения будем разбирать на 2м занятии)
