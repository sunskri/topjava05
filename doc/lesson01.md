...
- Хранение в HashMap - одна из наших реализаций (такжен будет jdbc, jpa и data-jpa). Т.е. с ней лучше работать через интерфейс, который НЕ ДОЛЖЕН НИЧЕГО знать про о деталях реализации (Map, DB или что-то еще). Пока нет Spring, можно его инстанциировать в сервлете (слои приложения будем разбирать на 2м занятии).
