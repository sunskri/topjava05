...
- Хранение в HashMap - одна из наших реализаций (такжен будет jdbc, jpa и data-jpa). Т.е. с ней лучше всего работать через интерфейс и сервлет не должен знать о деталях реализации (Map, DB или что-то еще, слои приложения будем разбирать на 2м занятии)
